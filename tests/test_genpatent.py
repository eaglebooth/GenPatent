...
CONTRACT = ROOT / "contracts" / "GenPatent.py"


class GenPatentContractStaticTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_contract_has_nondet_consensus_methods(self):
        ...
        self.assertIn("def file_patent", self.source)
        self.assertIn("def evaluate_patent", self.source)
        self.assertIn("def evaluate_infringement", self.source)
    ...
    def test_storage_annotations_use_allowed_types(self):
        contract = next(
            node
            for node in self.tree.body
            if isinstance(node, ast.ClassDef) and node.name == "GenPatent"
        )
